...
use crate::components::{
    ...
    Morale, Order, Path, Pos, PrevFacing, PrevPos, RangedState, Regiment, RegimentFatigue, SlotRef,
    Vel,
};
use crate::resources::{
    Clock, Events, Ids, LastHash, MoraleShocks, PendingDamage, Phase, Projectiles, Rng, Sides,
    StepEvents,
};

/// Hashes exactly the fields SIM-DET-004 lists (as amended in T1-047,
/// T2-020, T2-030 and T2-040), in that order: tick; phase; per side
/// (ascending index) escape edge, general, general regiment, general dead;
/// per regiment (ascending id) morale, morale state, soldier count, anchor,
/// order (kind, target, target regiment, facing, speed, since), formation
/// (template, ranks, files, integrity, morph_until, needs_reform, prior
/// template, laid-out facing), path (waypoints with corridors, next,
/// requested), fire state (present for ranged units: mode, target,
/// cooldown), combat (engaged, last fighting, charge_until, experience,
/// kills, fled), casualty ring, initial strength, rout count, engaged
/// since, arc hits, fatigue mean; per soldier (ascending id) `p`, `v`,
/// facing, `hp`, `fatigue`, FSM state, slot, melee (target, cooldown),
/// ranged state (present for ranged units: ammo, cooldown), general rank
/// (present for the general); per projectile (ascending id) every launch
/// field; pending damage in queue order; morale shocks in queue order; RNG
/// stream states.
pub fn compute_hash(world: &mut World) -> StateHash {
    ...
    for entity in regiment_entities {
        let (regiment, morale, anchor, order, formation, path, combat, fire, fatigue) = regiments
            .get(world, entity)
