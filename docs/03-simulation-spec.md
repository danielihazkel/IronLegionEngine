...
| SIM-AI-011 | On `attack` (as tuned in T2-082 against the §15.3 rows 9 and 10 and reworked in T3-010) the line forms where its regiments stand (the line regiments' centroid; `arrange_group` on a battle-line template with `formation.group_gap`, its ranks chosen to match six tenths of the visible enemy *line's* frontage so that, centred on the strike point, it overlaps the wing it strikes and that wing's neighbour rather than one regiment of theirs each, facing the target; the enemy line is its infantry when any is in sight, else everything visible) and steps `ai_profile.advance_step` per period toward the *strike point*, the enemy line's nearer wing (`0.6 ×` that line's lateral half extent from the enemy centroid on the side of the own centroid), while its regiments lag their slots by less than twice `line_tolerance`; the enemy frontage it matches is capped by the room the map offers across the line (the map rectangle's extent along the line's lateral axis less a `group_gap` at each end), and a line that is still wider than that room at its deepest ranks folds onto a `double_line` (SIM-FORM-042 geometry) instead of spilling past the edges (T3-010). It holds at `approach_distance` from the strike point while its missile units stand off with ammo (the archers' stand-off is out of the enemy's reach) or while the line's mean fatigue is above the fresh threshold, for at most `ai_profile.standoff_max_ticks` from the tick it reached the approach distance (`ArmyPlan.standoff_since`, kept while the stance lasts; T3-010), then closes to 10 m; regiments march to their slots (`SetSpeedMode` never; `Move` at `March`, walk pace at half the fatigue) and `engage_nearest` takes over as the lines close. Line regiments hold their slots and do not pick fights of their own until the plan charges (an enemy that engages one of them is fought at once). Once a line regiment is within `charge_trigger_dist` of a visible enemy and the line is dressed (its regiments lag their slots by less than twice `line_tolerance`), or as soon as the enemy has engaged a line regiment, the plan is `charging` (sticky for the stance; T3-010: a line that trickled in one regiment at a time lost each fight one at a time): the flank groups, which hook round the struck wing (`flank_offset` beyond the enemy line's extent on the strike side), run and charge the enemy line regiment nearest the strike point from behind (`AttackRegiment`; the rear-most visible regiment when no line regiment is in sight); the line and the reserves run too if the line's mean fatigue is under `ai_profile.charge_max_fatigue` when the charge begins (`ArmyPlan.run_in`, sticky once set; T3-010), else they walk in and the sim's own charge distance (SIM-CMBT-004) runs the last stretch. Missile units with ammo stand off at `skirmish_range_frac × unit.ranged.range` from the nearest visible enemy *line* regiment (infantry or cavalry; the nearest enemy of any kind when no line is in sight) on the line toward it, and aim their volleys at the nearest such regiment within their range (`FireMode::Target`, back to fire at will when none is in reach; T3-010: measured from and shot at the loose screen, twenty volleys killed two men), unless an enemy missile unit reaches that point (its range plus 10 m) and the charge is not on. Otherwise bows stand half a `reserve_offset` behind the line, shooting over it, and javelins with ammo wait `formation.skirmish_offset` ahead of it, so that they lead the charge and take the enemy's pila and javelins in loose order instead of the line (T3-010); once the charge is on the javelins take their stand-off whatever the fire. A spent missile unit stands a `reserve_offset` behind the line and never joins an attacking line (T3-010: it used to, and was destroyed in the melee for nothing); on defend every missile unit stands in the line. Cavalry flank groups head at `run` for the enemy's nearer flank, `flank_offset` beyond the visible enemies' lateral extent, pulled back along their own approach to passable ground. `approach_distance` doubles as the campaign's deployment distance in Phase 4 (plan decision 22). | REQ-AI-003 |
...
| SIM-AI-014 | Reserves hold `reserve_offset` behind the line centre, spread by their widths plus `formation.group_gap`; every period a reserve is committed (`Committed { target }`, an `AttackRegiment` at walk) to the nearest visible enemy of the line regiment with the lowest morale below `ai_profile.commit_morale`, and one more to each visible enemy that threatens a flank (its lateral offset beyond the line's half width by at most `flank_offset`, and no farther ahead of the line than that: the enemy regiments a narrower attacking line does not strike stand beyond its ends and count once level with it, T3-010); a commitment stands while its target is visible and alive. | REQ-AI-003 |
...
| The engine AI (side 1) against a passive player army (side 0, no commands; 3 × 120 hastati, 120 velites, 60 archers, 60 cavalry and a 20-man bodyguard a side, pre-placed 250 m apart; `ai_vs_passive`) | The AI wins in at least 70 % of seeds (`winner` side 1, 50 seeds, `min_fraction` 0.7): the Phase 2 exit criterion as the owner set it on 2026-09-10 (T3-011; it read "every seed of 20" until then, and stood unmet at 36/50 after T3-010, so rows 9 and 10 now read alike). Measured 2026-09-06 in release: 3/20; after the T2-082 tuning pass of 2026-09-07 3/20 (a smaller passive army, without its archers and one hastati regiment, 13/20): an open target, the T2-082 finding in docs/07 (the assault charges at fatigue 0.4 after a wasted stand-off and is exhausted within a minute of melee against a fresh line). T3-010 (2026-09-10) rebuilt the attack (a rest at 100 m until the line is fresh, one concentrated strike on the enemy line's wing with the cavalry on it, the velites screening the charge, the missiles aimed at the line) and measured 36/50 in release; the losses left are seeds where the defenders' pila and the contact shock break the lead regiment in the first twenty seconds and the rout spreads, see the T3-010 record in docs/09. | — |
| The same armies, the player charging everything straight ahead at tick 1 (`AttackMove` for every regiment; `ai_vs_charge`) | The AI wins in more than 60 % of seeds (50 seeds since T3-011, 20 before; `min_fraction` 0.6). Measured 2026-09-06 in release: 16/20 (borderline; the same finding); 41/50 on 2026-09-10 after T3-010. | — |
| Determinism | Every scenario above: identical hash on run 1 and run 2, on one thread and eight, and after snapshot/restore at tick 1,000 of a 1,500-tick run (each band file's `determinism` block; T2-112 enrols them through the band harness's driver, so the morale pins and the general kill apply; `tests/scenarios/ai_skirmish_300.json5`, both sides the engine's, sits in the classic corpus since T2-082, and the 10k fight `perf_10k.json5` runs 800 ticks with the restore at 400). Rejected commands: none in a file whose commands are all scripted; a file with an engine-owned side (rows 9 and 10) may reject a few through the one-tick race of SIM-CMD-005 (the AI decides at Stage 1 of `t` for `t + 1`, and the target can rout or die in between), and the nightly runs those files twice with the same seeds and requires the same count on the same ticks and the same final hash in every seed (T3-011). | — |
...
| `approach_distance` / `advance_step` / `line_tolerance` | 100 (T3-010: from 150, so the rest before the charge sits just outside the enemy skirmish screen's reach; at 150 the last stretch re-tired the line) / 8 / 12 m | SIM-AI-011 (the line steps `advance_step` toward the enemy per army period once its mean slot error is under `line_tolerance`) |
| `skirmish_range_frac` / `flank_offset` / `charge_trigger_dist` | 0.9 / 40 m (tuned from 80: the wider hook fell into the test map's river) / 40 m (T2-082 raised it to 60 so the line ran the last stretch under fire; T3-010 took it back to 40, since a hastati regiment gains 0.02 fatigue a second at a run and the sim runs the last 30 m by itself, SIM-CMBT-004) | SIM-AI-011 |
| `standoff_max_ticks` / `charge_max_fatigue` | 2,400 ticks / 0.15 (*chosen* in T3-010: the stand-off's cap, and the mean line fatigue the line rests under before closing and runs its charge under; a hastati regiment gains 0.0036 a second marching, its armour counted, and recovers 0.01 a second standing) | SIM-AI-011 |
