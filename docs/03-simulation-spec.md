...
| SIM-CORE-005 | Each Regiment has: stable `RegimentId`, `ArmyId`, `UnitType` handle(s), soldier list, anchor `(a, θ_a)`, `FormationTemplate` handle, formation state, current order, path, `morale`, morale state, speed mode (walk/run/march), `experience`, ability cooldowns, status effects, `ammo` (ranged), engagement flags. | REQ-VIS-003 |
...
| SIM-CORE-008 | Soldiers are removed from the world at death after `combat.corpse_ticks` (render-only; the sim removes them from all queries immediately). | — |
...
| SIM-CORE-011 | Transitions: `Idle ↔ MoveToSlot` when distance to slot crosses `movement.slot_arrive_radius` (enter Idle) or `movement.slot_leave_radius` (enter MoveToSlot); `→ Fighting` when a melee target is within reach (§6); `Fighting → MoveToSlot` when the target is lost and no other enemy within `combat.engage_radius`; `→ Routing` when the Regiment enters Routing (§7); `Routing → MoveToSlot` on Rally; `→ Withdrawing` when the Regiment withdraws; `→ Dead` when `hp ≤ 0`. | REQ-AI-002 |
...
| SIM-DET-005 | A snapshot contains everything the hash covers plus everything needed to continue: paths, cooldowns, status effects, timers, flow fields are recomputed on restore (derived data is never stored). Restoring and stepping shall produce the same hash sequence as the uninterrupted run. | REQ-SIM-006 |
...
| SIM-FORM-005 | **Square**: four Line segments of `n/4` soldiers each facing outward, forming a hollow square with side `ceil(sqrt(n)) · sf`; remainder fills the rear side. Slot facing is the segment's outward normal. | REQ-FORM-002 |
...
| SIM-FORM-042 | The player's drag-formation gesture produces a `GroupFormation { battle_line, width }`: the engine chooses `ranks` per regiment so that the total width matches the drag width within `formation.width_tolerance` (default 10 %), clamped to `[min_ranks, max_ranks]`. | REQ-FORM-010, REQ-INP-003 |
...
| SIM-MOVE-004 | A regiment wider than the narrowest passable corridor along its path (measured as passable run length at each waypoint's cell) automatically morphs to Column for the corridor and back afterwards, unless the order set a template explicitly. | REQ-PATH-006, REQ-SIM-042 |
...
| Rule | Statement | Satisfies |
|---|---|---|
| SIM-PROJ-001 | A ranged regiment in `fire_at_will` selects, every `combat.ranged_retarget_ticks` (default 10), the enemy regiment with the most soldiers inside its range annulus `[min_range, range × range_mult]` and line of sight (indirect arc ignores LOS occlusion by soldiers but not terrain), preferring the current target if still valid. `target` mode uses the ordered regiment; `hold` fires nothing. | REQ-CMBT-013 |
| SIM-PROJ-002 | `range_mult = 1 + combat.height_range × clamp((h_shooter − h_target) / combat.height_ref, −1, 1)` (default 0.2). | REQ-CMBT-014 |
| SIM-PROJ-003 | Each soldier with `ammo > 0`, not `Fighting`, whose regiment has a target, fires when its reload cooldown reaches 0 (reset to `unit.ranged.reload_ticks × fatigue_interval_mult`), aiming at the position of a target soldier chosen deterministically (`hash(tick, id) % target_regiment.count`-th soldier by ascending id) predicted forward by flight time; a regiment fires in volleys by sharing the cooldown phase (`combat.volley` true: all soldiers' cooldowns are synchronised to the regiment's). `ammo −= 1`. | REQ-CMBT-010, REQ-CMBT-011 |
| SIM-PROJ-004 | Scatter: the aim point is offset by a vector with angle `rng.combat_ranged(tick, id, 0) × 2π` and length `d × (1 − unit.ranged.accuracy) × combat.scatter_scale × weather.accuracy_penalty` (default scale 0.15) where `d` is the distance. | REQ-CMBT-011 |
| SIM-PROJ-005 | Projectile motion: `direct` arc flies straight at `projectile_speed` with height following a shallow parabola (`h = h0 + apex × 4t(1−t)`, apex `combat.direct_apex` default 2 m); `indirect` arc chooses launch angle 45° with speed adjusted so range equals `d` (capped at `projectile_speed`), integrating `p += v_xy × dt`, `z += v_z × dt`, `v_z −= combat.gravity × dt` (default 9.81). Flight time is precomputed at launch so both arcs are deterministic; a projectile lands when its precomputed `land_tick` arrives. | REQ-CMBT-010 |
| SIM-PROJ-006 | Landing: query the spatial grid at the landing point for soldiers (any side) with `|p − land| ≤ r + combat.projectile_radius` (default 0.3 m); the nearest by distance (ties ascending id) is hit. A hit applies `dmg = max(ranged.damage − armour × (1 − ranged.armour_penetration), min_damage)`, with rear/flank multipliers by the impact direction. Shields: if `unit.shield` and the impact is frontal, `dmg × combat.shield_mult` (default 0.5). | REQ-CMBT-010, REQ-CMBT-012 |
...
| SIM-PROJ-009 | Friendly soldiers at the landing point are hit like enemies; indirect fire may be ordered over friendly regiments; direct fire is refused (`hold` with Event) if a friendly regiment's footprint intersects the line of fire within `combat.friendly_block_dist` (default 15 m) of the shooter. | REQ-CMBT-012, REQ-CMBT-013 |
...
| Field | `rome:hastati` | `rome:velites` | `greece:hoplite` | `persia:cavalry` |
|---|---|---|---|---|
| category | infantry | skirmisher | infantry | cavalry |
| soldier_radius / mass | 0.4 / 80 | 0.4 / 70 | 0.4 / 85 | 0.7 / 400 |
| hp | 100 | 80 | 110 | 160 |
| speed_walk / run / march | 1.6 / 4.0 / 1.6 | 1.8 / 4.5 / 1.8 | 1.4 / 3.6 / 1.4 | 3.0 / 9.0 / 3.0 |
| attack / defence / armour / damage | 35 / 30 / 8 / 30 | 25 / 20 / 2 / 25 | 32 / 38 / 10 / 30 | 38 / 25 / 8 / 35 |
...
| cost / upkeep / recruit_turns / regiment_size | 400 / 60 / 1 / 120 | 250 / 40 / 1 / 120 | 450 / 70 / 1 / 160 | 800 / 120 / 2 / 60 |
