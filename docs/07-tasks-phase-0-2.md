...
- [x] 2,000 soldiers in 10 regiments move, wheel, and reform at 60 FPS with the sim tick at or under 10 ms on the target machine (profiler overlay screenshot saved under `docs/evidence/phase1/`: `profiler_2000_moving.png`, 60 FPS, tick 6.07 ms mean over 60 ticks in the app; `il_cli bench --soldiers 2000` 4.0 ms; machine in `machine.md`).
- [x] A mod folder overriding `speed_walk` takes effect with no code change (T1-082; `tests/tests/mod_override.rs`, `--mod tests/mods/speed_override` in il_app).
- [x] Determinism test passes for `idle_1000` and `move_reform_2000` at 1 and 8 threads, including snapshot/restore (`tests/tests/determinism.rs`, 2026-09-03; CI repeats it with `il_cli run` in release).
- [x] `il_cli validate game/` is clean in CI (run 33835342644 on `main`, commit 3272fdc, 2026-09-04: 0 errors, 0 warnings; the same run repeated the determinism proof in release and the bench comparison).
- [x] Every Phase 1 Must requirement is satisfied: REQ-VIS-002, 003, 004, 021, 023, REQ-PERF-001, 004, REQ-TECH-003, 004, 005, REQ-SIM-020, 022..025, 041, 045, REQ-FORM-001..005, 010, REQ-PATH-002, 003, 005, 006, 009, REQ-AI-002, REQ-RNDR-001, 002, 003, 005, 006, 008, REQ-INP-001..004, 006, REQ-LOC-001, 003, REQ-MOD-001, 004..007, REQ-TOOL-002, 003, REQ-TEST-001, 005.
  Audited 2026-09-03 against the code. REQ-LOC-001 failed the audit (English literals in the egui panels and the window title) and was fixed in the close-out: every engine label is an `il.*` locale key and `tests/tests/locale_keys.rs` keeps it so. Requirements whose remaining clauses belong to later phases are met to the extent their systems exist: AI-002 (only Idle and MoveToSlot transitions run until combat), PATH-005 (fatigue does not scale speed until Phase 2), RNDR-008 (morale and LOS overlays arrive with their systems), INP-003 (attack-move, ability and withdraw bindings arrive with Phase 2: abilities in T2-050, attack-move and withdraw in T2-090), MOD-001 (technologies, buildings, abilities, AI and audio kinds arrive with their phases), TEST-001 (every Phase 1 formula has a test), VIS-021 (Lua is Phase 6), SIM-045 (`structures` and `siege_points` are reserved as untyped lists). Two deliberate deviations are recorded in the TDD: RNDR-003 interpolates positions but snaps facings (§10.1), RNDR-006 colours zones from a palette rather than tile textures (§10.1 terrain).

Phase 1 completed 2026-09-04: CI run 33835342644 on `main` (commit 3272fdc) is green; every box above is ticked. Still owed to the owner's eyes: the in-window drag-formation check (`docs/08-how-to-run.md` §4).
...
- [x] **T2-010 Combat, morale, fatigue, general, visibility, battle-flow rules and content** · M · Refs Simulation Spec §15.1, TDD §8.1 `CombatRules`, §8.3 `MoraleRules`, `FatigueRules`, CONTENT
  ...
  **Done when** `il_cli validate game/` is clean and every rule field is read by at least one system by the end of the phase (grep check).
  Done 2026-09-04: eight rules structs in `il_data::rules` (every field required, `Rules::hash_content` covers all), six new `rules-*.schema.json`, `game/content/rules/{combat,morale,fatigue,general,visibility,battle_flow}.json5`; §15.1 rewritten as one table per file with the prose-only tunables added (*chosen* marks values no rule stated); zone types gained `defence_mult` and `ford`, units a top-level `armour_penetration`; regiments now spawn with `unit.ranged.ammo` (hash goldens re-baselined). The grep check became `tests/tests/rules_fields_read.rs` in T2-113: every schema property must be read by a system in `il_sim_battle` or `il_ai`, or sit on the test's allow-list with its reason (`corpse_ticks` render-only; `hpa_cluster`, `hpa_gate_split` Phase 3; `fled_return_fraction` Phase 4; the `state_mults.*` entries read through `StateMultsTable::for_state`).
...
- [x] **T2-030 Projectile pool, ranged targeting, firing** · M · Refs SIM-PROJ-001..004, TDD §8.2, REQ-PERF-008
  `Projectile` records in a `Projectiles` list reserved once at `projectile_cap` (no entities: the position is closed-form from the launch data); `Fire` regiment component with `FireMode`; ranged retarget every `ranged_retarget_ticks`; per-soldier `RangedState` reload and ammo, volley synchronisation, aim prediction, scatter via `hash_draw`, direct-fire friendly block check.
...
- [x] **T2-041 Morale value, factors, states** · L · Refs SIM-MOR-001..027, TDD §8.3 `morale_factors`, `morale_state`
  ...
  **Done when** factor unit tests and the hysteresis table test pass, and a regiment losing 30 % in 10 seconds passes through Broken into Routing.
  Done 2026-09-05: `morale/factors.rs` (pure factors, delta, shock amounts, hysteresis; unit tests) and `morale/tick.rs` (Stage 14 gather-then-apply, `MoraleChanged` event); shock producers at Stage 0, Stage 9 and Stage 10's arc stamps; regiments spawn in the state their morale falls into (hastati 60 → Unsettled); `F10` morale overlay; `tests/morale.rs` (recovery, the 30 % break, hysteresis, allies, charge and disengage shocks, 1 vs 8 threads with a restore). Factor values are activations with the sign in the weight (the spec had double-signed them). Routing regiments stop fighting (`melee_gate`) but do not flee until T2-042.

- [x] **T2-042 Routing, rally, shatter, escape flow fields, contagion, pursuit hits** · M · Refs SIM-MOR-030..034, SIM-FLOW-001..003, REQ-PATH-004
  ...
  **Done when** a broken regiment flees to its own edge along passable cells, rallies when safe, and shatters on the second rout.
  Done 2026-09-05: `flow.rs` (per-side escape field over the shared `NavGrid::for_each_neighbour` graph, `escape_edge` from the deployment polygon: South for zone 0 and North for zone 1 on `rome:test_field`), `morale/rout.rs` (`enter_routing` with the shatter test and `Rout` shocks, `try_rally`, `follow_centroid`), the `flee` steering branch, Stage 15 `resolve_fled` (`SoldierFled`, `Combat.fled`), the pursuit hit multiplier and cavalry running after routers, `F11` flow overlay; the band harness counts the fled apart from the dead; rows 1 and 4 rout clauses active, row 2 gains its 90 % ceiling; `tests/rout.rs` (flight to the edge, rally, second-rout shatter, contagion, pursuit, 1 vs 8 threads with a restore). Older fight tests pin morale at 100.

- [x] **T2-043 Generals and auras** · M · Refs SIM-GEN-001..005, REQ-CMBT-020..023
  ...
  **Done when** the general-death scenario band (row 7) holds.
  Done 2026-09-06: `GeneralSetup.bodyguard` (default the side's first regiment), the general spawned as that regiment's extra last soldier with its own unit (`rome:general`, `greece:general`, `persia:general`, category enforced), `hp × hp_mult` and `GeneralTag`; `MeleeGateRes.in_aura` at Stage 9 feeding `aura_attack_mult` and the `general_aura` factor; `GeneralDied` at Stage 15 with the side-wide shock; `BattleWorld::general_fate`; the aura circle on `F10`; every fixture switched to the general units; charge mass now per soldier unit (a general kept its regiment's mass after a charge and restores diverged). Harness `harness: [{ tick, kill_general }]` and `routs_first`; row 7 `general_death_hastati_vs_hoplites.json5` (mirrored hastati 100 m apart, kill at tick 600). `tests/general.rs`.

- [x] **T2-050 Abilities: buff and debuff, status effects, energy** · M · Refs SIM-ABIL-001..007, REQ-ABIL-001..003
  ...
  **Done when** testudo reduces arrow casualties in a scenario and expires on time.
  Done 2026-09-06: `il_data::ability` (`Ability`, `Effect` tagged by `type`, `Targeting`, `Stat`, `Stacking`; non-executable kinds and zero durations rejected in `resolve`), `ability.schema.json`, `Registries.abilities`, `UnitType.abilities` resolved to handles plus `energy_max`/`energy_regen`; `il_sim_battle::abilities` (`use_ability` at Stage 0 with `RejectReason::Ability { why: AbilityFail }`, `status`, Stage 12 `ability_tick`), `Energy`/`Statuses`/`Cooldowns` components, `StatMults` cached per regiment and wired into melee, ranged, statistical shots, soldier and anchor speed, fatigue and the morale tick; the milestone's whole layout declared (`Combat.withdrawn`, side flags, `BattleFlow`, `Visibility`; `SNAPSHOT_VERSION = 7`, goldens re-baselined); `rome:testudo` (armour ×3.5 after tuning from ×2), `greece:shield_wall`, `persia:war_cry`; Z/X/C hotkeys and the card's ability and status lines; harness `mean_loss_below` and row 8 `volley_testudo.json5`; `tests/abilities.rs`.

- [x] **T2-060 Line of sight and fog of war** · M · Refs SIM-VIS-001..006, REQ-SIM-050..053, TDD §8.4
  ...
  **Done when** a regiment behind a hill is not targetable and becomes visible when the hill is crested.
  Done 2026-09-06: `visibility.rs` (per-side masks and memory, `los_radius`, `segment_clear`, `regiment_visible`, Stage 8 `visibility_update` staggered by side, `recompute_all` in `new`); `LoadedMap.mean_height`; `NotVisible` for `AttackRegiment` and `FireMode::Target`, hidden targets refused by `UseAbility`, ignored by fire-at-will and `AttackMove`; `BattleView::{visible, visible_regiments, seen, los_radius}`; the window hides what the local player's side cannot see and draws ghosts, `Ctrl+F5` overlay; `tests/visibility.rs` (the hill pair found by scanning the map, the forest, stagger and memory, `AttackMove`, 1 vs 8 threads with a mid-period restore) and a render fog test. Keyed by side, not faction (plan decision 12); melee and morale unfiltered (decision 14). The fatigue determinism fixture now attack-moves: on ground below the map's mean height a hastati regiment's radius is about 184 m, so its tick-1 `AttackRegiment` at 200 m was refused as `NotVisible`. Bands unchanged (13/13, the volley means as in T2-050).

- [x] **T2-070 Battle phases: deployment, battle, pursuit, ended** · M · Refs SIM-FLOW-010..017, REQ-SIM-030..034, 036
  ...
  **Done when** a scripted battle passes through all four phases and emits `PhaseChanged` events in order.
  Done 2026-09-06: `flow_battle.rs` (Stage 16 `battle_flow`: engine-AI confirmations, the deployment end and timeout, reinforcements in Column at their edge, defeat detection, Pursuit and Ended with the timeout verdict of plan decision 18), `Stage::runs_in` (Deployment runs stages 0/6/8/16/17, Ended 0/17), `result::compute` behind `Ended { result }`; `Deploy` zone check (`OutsideDeploymentZone`), `ConfirmDeployment`, `Surrender`, `Withdraw` (soldier-driven march at March, `Combat.withdrawn`, `SoldierWithdrew`); `ReinforcementGroup.edge: MapEdge` validated against the map; `RegimentSetup.position` kept as the pre-deploy override (OQ-9): fully placed setups start in Battle as before, others auto-place a battle line at the zone centre and start in Deployment. App: right-click/drag deploys, Enter confirms, phase line, result window, stepping stops at Ended; `il_cli run` prints `# ended`. `tests/phases.rs`, `tests/scenarios/phases_all_four.json5` in the determinism corpus, `tests/mods/timeout_most_soldiers`. The melee-target attack fixture moved to `close_sides` (a fog miss of T2-060). Restore no longer rederives `Combat.engaged` from the FSM (it is stored state read at Stage 9; Stage 14 may rout the soldiers later in the same tick), which a mid-pursuit restore exposed. A battle with fewer than two sides has no verdict, and a side whose every regiment routs is defeated at once even if it rallies later.

- [x] **T2-071 `BattleResult`, scenario files, `il_cli autoresolve`** · M · Refs SIM-FLOW-018, 019, REQ-SIM-061, 063, TDD §9 auto-resolve path
  ...
  **Done when** result totals reconcile with initial counts for every scenario in the test set.
  Done 2026-09-06: `result::compute` completed (experience points per plan decision 19, ammo as the living soldiers' sum, the general's fate with `lost` = another side won, loot for the winner, `arrived` for pending groups) behind `BattleWorld::result()`, available at any tick with `winner` only once Ended (decision 23); `il_cli autoresolve` (`autoresolve.rs`, scripted commands until T2-082, exit 2 when the cap cuts the battle short); the band harness stops a seed at Ended; SDK §4.13 scenario-file table. `tests/tests/result.rs` reconciles every scenario and band file; `tests/result.rs` and `crates/il_cli/tests/autoresolve.rs`. `BattleFlow.ended_at` freezes `duration_ticks` at the end (a second hash field this milestone; goldens re-baselined, `SNAPSHOT_VERSION = 8`).
...
- [x] **T2-081 Regiment AI and deployment** · M · Refs SIM-AI-020..022, TDD §8.5 inputs
  ...
  **Done when** an AI regiment forms Square against approaching cavalry and Phalanx when engaged frontally in scripted tests.
  Done 2026-09-06: `ai/inputs.rs` (`SideSnapshot` of own and visible enemy rows per tick, `RegimentContext: InputProvider` for the SIM-AI-021 inputs, distances raw in metres), `ai/regiment.rs` (movement, formation, fire and per-slot ability channels with the applicability rules, command hygiene, `Flank`/`Committed` role overrides, the bodyguard's `follow_centroid`), `ai/deploy.rs` (SIM-AI-020 battle line with skirmishers ahead, cavalry on the flanks, the bodyguard behind; `Deploy`s plus `ConfirmDeployment`), Stage 16's engine-AI auto-confirm removed; `il_app --ai <player>`, `Ctrl+F6` `debug_ai` overlay and title stances, AI commands in the session log; `tests/ai.rs` (square at 40 m, phalanx within two periods of `Engaged`, deployment geometry, engage once, hold fire behind a friend, testudo under javelins, fall back, the bodyguard, zero rejections over a fight, 1 vs 8 threads with a restore, replay with the AI off). Movement bases retuned (engage 1.5, hold 0.25) so an even enemy inside about 25 m draws an attack.
...
- [ ] **T2-082 Army AI** · L · Refs SIM-AI-010..014, REQ-AI-003
  ...
  **Done when** the AI army beats a passive player army (Phase 2 exit criterion) in 100 % of 20 seeds and beats a "charge everything straight ahead" scripted opponent in over 60 %.
  ...
  Tuning pass 2026-09-07 (plan decision 2, six iterations of 20 seeds on `ai_vs_passive`, each lever a mod folder on top of `game/`): the morale levers `w.casualty_rate` −4 with `casualty_rate_ref` 0.08 gave 9/20, `w.winning` +3 with `safe_radius` 80 gave 4/20, `charged_penalty` 12 gave 3/20; the profile levers `aggression` 0.8, `reserve_fraction` 0 and `charge_trigger_dist` 40 gave 5/20 alone and 6/20 with the first morale lever; resting closer (`approach_distance` 100, `skirmish_range_frac` 0.85, `charge_trigger_dist` 40, `advance_step` 12) with that lever gave 0/20, the line resting under the enemy archers. The fallback army of plan I15 (the passive side without its archers and with two hastati regiments, 441 against 621) gave 13/20 with the rules as they were and 16/20 with the first morale lever (alone or with the profile levers). Nothing kept: the best lever (`w.casualty_rate` −4 with `casualty_rate_ref` 0.08) lifted the passive band to 9/20 (15/50) and left rows 1 to 8 holding at 50 seeds (`ai_vs_charge` 43/50), but it breaks T2-041's done-when (a regiment losing 30 % in 10 seconds no longer passes through Broken into Routing: `tests/morale.rs`), an accepted rule, so it is recorded for the owner's decision and the rules stay as they were. What the timelines show (a throwaway per-200-tick dump of every regiment's count, morale state, order, position, fatigue and plan role): the army walks to `approach_distance` in about two minutes, then rests there for a further two to three minutes because its archers' stand-off slot lies 0.9 × range from the *nearest* enemy (the loose velites in front of the line), where twenty volleys kill two men; the line only closes once the archers are spent and joined to it; it charges at fatigue 0.4 from the run-in and reaches 1.0 inside a minute of melee (`rate_run` 0.020/s, `rate_fighting` 0.015/s) against a line that started at 0, so the whole assault routs even where it was winning locally. The fatigue rates and the stand-off rule were outside the agreed levers; the criterion stays open for the owner's decision (relax it, rework the stand-off and the fatigue rates, or carry it into Phase 3).

### UI — battle UI and menus

- [x] **T2-090 Battle UI** · L · Refs REQ-UI-001, 003, 006, TDD §11 panels
...
- [x] **T2-111 10k performance pass** · L · Refs REQ-PERF-002, TDD budget table
  ...
  **Done when** the benchmark baseline is updated and CI's +20 % gate is green at 10k.
  Done 2026-09-07: `tests/scenarios/perf_10k.json5` (5,000 a side in 25 regiments of 200 plus bodyguards; side 0 attack-moves into the engine AI's side 1 at tick 20, because two engine armies never closed: a 25-regiment battle line is wider than the 800 m map and the army AI's advance never settles, a Phase 3 item for the army AI); `il_cli bench --scenario <file>` (keyed by the file's stem, 1,200 ticks by default, stops at `Ended`); the fight measured 14.3 ms mean / 19.6 p95 before and 13.3 / 18.3 after the semantics-preserving fixes (the melee gate's extents from one table scan, anchor-grid queries with a caller scratch, the targeting pass writing only on change, the fatigue tick's regiment table, the morale tick's per-entry side table and parallel gather), with the `il_cli run` hash logs of `perf_10k`, `ai_skirmish_300` and `move_reform_2000` identical before and after; every combat stage inside its 20k budget at 10k (Targeting 1.5, Morale 0.6, Fatigue 0.4 ms); Collision and Steering stay Phase 3 (SAD T-10). Baseline re-recorded for every key in one sitting (3.52 / 14.45 / 30.50 ms at 2k / 10k / 20k, `perf_10k` 13.33 ms); `--strict` passes on the target machine by construction; CI runs the `perf_10k` comparison warn-only after the 2k line. `docs/evidence/phase2/{machine,bench_perf_10k}.md`; TDD budget table Phase 2 columns. Pulled forward from T2-112 so this commit's test suite stays short: `Scenario.determinism: { ticks, snapshot_at }` (default 10,000 / 5,000) and the determinism test reading it; `perf_10k.json5` runs 800 / 400 in the debug build.

- [x] **T2-112 Determinism coverage** · S · Refs REQ-TEST-002
  ...
  **Done when** they pass.
  Done 2026-09-07: `full_combat_10k` is `tests/scenarios/perf_10k.json5` (T2-111); the ten band files carry `determinism: { ticks: 1500, snapshot_at: 1000 }` and run through the band harness's `SeedDriver` (`il_cli::bands`, shared with `run_seed`: the general kills before a tick, the morale pins after it, the hash after the pins), so the corpus is what the nightly measures; the 10k fight runs 800 / 400 in the debug build and 1,500 ticks in CI's release double-run (`--hash-every 100`). `tests/tests/determinism.rs` holds three corpora as three tests (classic, bands, the 10k fight) so cargo runs them side by side; band and perf files, AI-driven with the documented one-tick race, assert identical rejection counts per tick across the compared runs instead of none (plan I13). The whole debug-build test file takes about 8 minutes on the target machine (docs/08 §8).

- [x] **T2-113 Docs update** · M
  ...
  Done 2026-09-07: Simulation Spec §15.1 and §15.3 carry every tuned default and the last measurement of every row (the T2-082 pass included); the TDD budget table holds the Phase 2 columns (T2-111); no new ADR (the milestone changed no architecture: SAD §12 T-5 gains the main-thread note, T-10 the fight numbers, T-11 and T-12 the audio crate); OQ-6 was resolved by T2-060 and OQ-8 by T2-100, both marked in the PRD, whose §30 records Phase 2's state; `tests/tests/rules_fields_read.rs` closes T2-010's grep check; `docs/evidence/phase2/` holds `machine.md`, `bench_perf_10k.md`, `bands.md` and the owner's `profiler_10k.png`; the Must-requirement audit sits under the exit checklist.

### Phase 2 exit checklist

- [ ] 10,000 soldiers fight to a conclusion at 60 FPS with the sim tick at or under 25 ms (profiler screenshot under `docs/evidence/phase2/`). Headless: `perf_10k` at 13.3 ms mean / 18.3 p95 in the fight (T2-111, `bench_perf_10k.md`); `autoresolve --ai none` runs it to its end by the timeout verdict. Owed to the owner: `profiler_10k.png` from `cargo run --release -p il_app -- tests/scenarios/perf_10k.json5 --threads 8` with F12 open during the melee; the FPS it shows is the evidence (plan decision 3: under 60, the main-thread sim of SAD T-5 is the recorded reason).
- [ ] The AI wins against a passive player (T2-082): `ai_vs_passive` 3/20 seeds on 2026-09-07 in release after the tuning pass (the fallback army of plan I15 13/20); see the T2-082 finding. Open: the owner's decision. Decided 2026-09-10 (T3-011): the criterion is at least 70 % of 50 seeds, the band file and Simulation Spec §15.3 row 9 carry it; T3-010 measured 36/50.
- [ ] Scenario band tests pass (T2-110): 2026-09-07 in release, `bands.md` (14 pass, 1 fail: the AI passive row). The nightly of 2026-09-07 to 2026-09-10 failed on the zero-rejection assert as well (T3-011 split it: scripted files none, AI-driven files the same rejections twice).
- [x] Determinism test passes with all combat systems at 1 and 8 threads, including snapshot/restore (T2-112): the classic corpus, the ten band files and the 10k fight, 2026-09-07.
...
- [ ] Every Phase 2 Must requirement is satisfied: REQ-PERF-002, 008, REQ-SIM-030..032, 035, 040, 060, 061, 063, REQ-FORM-006, REQ-PATH-004, REQ-CMBT-001..005, 007, 010..013, 015, 020..022, REQ-ABIL-001, 002 (buff/debuff), REQ-MOR-001..006, REQ-FAT-001..004, REQ-AI-001, 003, 005, 006, REQ-UI-001, 003, 006, 007, REQ-AUD-001, REQ-NET-008, REQ-SAVE-006. Met by the UI milestone (2026-09-06): REQ-UI-001, 003, 006 (T2-090), REQ-UI-004, 007 and REQ-INP-005's settings UI (T2-091), REQ-SAVE-005's recording and playback and REQ-SAVE-006 (T2-101: the save/load hash-equality test).
  Audited 2026-09-07 against the code. REQ-PERF-002: the 10k fight's tick 13.3 ms mean (`bench_perf_10k.md`; the 60 FPS half waits for the owner's screenshot). REQ-PERF-008: `projectile_cap` 8,192 with the statistical fallback (T2-030/032, row 6). REQ-SIM-030..032, 035: the four phases, `Deploy`/`ConfirmDeployment`, the timer and its verdict, `Withdraw` (T2-070, `tests/phases.rs`). REQ-SIM-040: `BattleSetup.reveal_deployment` (T2-060, OQ-6). REQ-SIM-060, 061, 063: `BattleSetup` / `BattleResult` / scenario files (T2-071, `tests/tests/result.rs`). REQ-FORM-006: `SetFacing` wheel versus about-face (T1-045) and the AI's formation switches (T2-081). REQ-PATH-004: escape flow fields (T2-042). REQ-CMBT-001..005, 007, 010..013, 015, 020..022: melee, ranged, projectiles and the cap, generals and auras (T2-020..043, rows 1 to 7). REQ-ABIL-001, 002 (buff/debuff): T2-050, row 8. REQ-MOR-001..006: T2-041/042, rows 1 to 4. REQ-FAT-001..004: T2-040, `tests/fatigue.rs`. REQ-AI-001, 005, 006: T2-080/081 (`crates/il_ai/tests`, `tests/ai.rs`); REQ-AI-003 (the army AI beats a passive player) is **open**: `ai_vs_passive` 8/50 and `ai_vs_charge` 36/50 after the T2-082 tuning pass, the finding recorded there. REQ-UI-001, 003, 006, 007: T2-090/091. REQ-AUD-001: T2-100 (`dep_rules.rs` keeps audio out of the sim; the router tests). REQ-NET-008: `TransferControl` (T0-031, the `--ai` hand-over in T2-081). REQ-SAVE-006: T2-101's save/load hash equality. Deviations on record: REQ-AUD-002 (zoom mixing) is a Should of Phase 3 and is built (T2-100); the 60 FPS clause of the exit criterion depends on the main-thread sim (SAD T-5); REQ-AI-003 as above.

The three open boxes above are closed by the carry-over tasks T3-010 to T3-012 in [09](09-tasks-phase-3-4.md#carry-over-from-phase-2) (the army AI rework, the nightly assert, the owner's screenshot); T3-012 writes the "Phase 2 completed" line here.

---

## After Phase 2

Phase 3 (scaling) and Phase 4 (campaign layer, MVP) have their own list, [09-tasks-phase-3-4.md](09-tasks-phase-3-4.md), written 2026-09-10; it opens with the carry-over tasks that close the three open boxes above.
