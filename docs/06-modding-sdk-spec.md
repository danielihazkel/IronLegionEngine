...
An **AI profile** is the personality a faction names in `ai_profile` (a scenario may override it per side, §4.13): `aggression`, `general_aggression`, `reserve_fraction`, `stance_margin`, the cadences `army_period_ticks` / `regiment_period_ticks`, the distances `approach_distance`, `advance_step`, `line_tolerance`, `skirmish_range_frac`, `flank_offset`, `charge_trigger_dist`, `standoff_max_ticks` and `charge_max_fatigue` (T3-010), `defend_search_radius`, `counter_charge_dist`, `screen_offset`, `screen_gap`, `reserve_offset`, `commit_morale`, and `action_sets` naming exactly one army set and one regiment set. Every numeric field is required (no engine defaults, like `content/rules/`); an optional `campaign` object (`army_strength_target`, `composition`, `min_garrison`) waits for Phase 4.

An **AI action set** lists the candidate actions of one `scope` (`regiment` or `army`). Each action is `{ name, kind, base, threshold, noise, considerations }`; `kind` is one of the closed behaviours (`engage_nearest`, `hold_position`, `fall_back`, `follow_centroid`, `use_ability` with `ability`, `switch_formation` with `layout`, `fire_at_will`, `hold_fire`; `attack`, `defend`, `hold`, `retreat`), and each consideration is `{ input, scale, curve }` with `input` from the closed vocabulary of the Simulation Spec §13 tables, `scale` the raw value that maps to 1, and `curve` one of `{ type: "linear", m, b }`, `{ type: "quadratic", k }`, `{ type: "logistic", k, mid }`, `{ type: "step", threshold }`. Unknown kinds and inputs are schema errors; an input of the other scope, a duplicate `name`, a `use_ability` naming a missing ability and a profile without one set of each scope are load diagnostics (§3.6). Action lists take the §3.4 list operations, so a mod may `$append` an action or `$replace` a set's list. The flagship regiment set (`rome:regiment_default`, T2-081): movement `engage_nearest` (base 1.5; distance to the nearest enemy over 80 m falling, strength ratio logistic, engaged 0.5..1), `hold_position` (0.25), `fall_back` (1.2; own morale logistic falling at 0.35, not engaged), `follow_centroid` (0.8); formation `square` (1.5, threshold 0.5; cavalry within 60 m, no infantry within 60 m), `phalanx` (1.2, threshold 0.5; engaged frontally), `line` (0.6, threshold 0.5); fire `fire_at_will` (0.8), `hold_fire` (1.0; a friend in the line of fire); abilities `testudo` (enemy ranged in range, not engaged), `shield_wall` (enemy ranged in range, or engaged frontally), `war_cry` (an enemy within 60 m), each threshold 0.5. The flagship army set (`rome:army_default`, T2-082): `attack` (1.5; strength ratio logistic at 0.45, aggression, morale mean), `defend` (0.8; terrain advantage, enemy visible), `hold` (0.4), `retreat` (1.2; strength ratio falling at 0.35, casualties rising at 0.35).
...
Actions: `camera_pan_up/down/left/right`, `camera_rotate_left/right`, `camera_zoom_in/out`, `camera_drag`, `select`, `select_add`, `box_select`, `box_select_add`, `select_type`, `select_all`, `group_set_0`..`group_set_9`, `group_recall_0`..`group_recall_9`, `order_move` (a right click on the ground moves, on a visible enemy regiment attacks it, T2-090), `order_drag_formation`, `order_flip_facing`, `order_halt`, `order_attack_move` (arms the cursor: the next `select` click on the ground attack-moves; Escape or a right click cancels), `order_withdraw`, `toggle_run`, `toggle_fire` (T2-030), `formation_1`..`formation_9` (the unit type's n-th formation template), `ability_1`..`ability_9` (the selection's ability slots, T2-050), `confirm_deployment` (T2-070), `pause`, `speed_up`, `speed_down`, `pause_menu` (opens the pause menu: resume, surrender, settings, quit; it replaced `quit_to_menu` in T2-090), `quick_save`, `quick_load` (battle quick save and load, T2-101), and in developer builds `toggle_profiler`, `debug_nav_grid`, `debug_slots`, `debug_paths`, `debug_anchors`, `debug_spatial`, `debug_morale`, `debug_flow`, `debug_los`, `debug_ai`.
...
| `sides[].regiments[]` | object | required | `{ id, unit_type, count, experience (0), fatigue (0), formation (the unit's first), position, facing_deg }`; a `position` pre-deploys the regiment (a fully placed side starts confirmed, a fully placed battle starts in the Battle phase), otherwise it is auto-placed at the zone centre and awaits `Deploy`. **Mixed regiments** (T3-002, SIM-FORM-012): `units: [{ unit_type, count, experience (0) }]` lists the unit groups in order (front to back without role zones; a unit type may repeat) and replaces `unit_type`, `count` and `experience`; a regiment gives one form or the other, never both. The composition is laid out by the template's `role_zones` (SIM-FORM-013) and reported per group in `RegimentResult.units` (SIM-FORM-015). Built in T3-040: the flagship's `rome:cohort` template (skirmishers in rank 1, infantry in ranks 2 to 8) is in the hastati's and velites' `formations`, and `tests/scenarios/mixed_cohort.json5` shows the form; a composition whose groups share no formation takes the first group's list with a warning `il_cli run` prints and the app lists in its event panel. |
| *schema* | | | `docs/schemas/scenario.schema.json` describes the whole file, band block included; `tests/tests/content.rs` validates every file under `tests/scenarios/` against it on every push (T3-002) |
