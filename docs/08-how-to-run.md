...
8. From the menu, without a scenario file (`cargo run --release -p il_app`): Custom battle, pick the map and two factions, set one side to You and one to Engine AI, add a regiment row or two (`+ unit` under a row adds a second unit group to that regiment, T3-041: a cohort of hastati and velites shows on its card as "Hastati 100 · Velites 40" and on the result screen as one row with a sub-row per group), Start. The battle opens in Deployment with your regiments in a line at your zone; pick a deployment preset or place them, Confirm, fight. Back in the builder, Save writes the setup under `tests/scenarios/<name>.json5` (the list under Scenario file shows it at once, and `il_cli autoresolve` runs it headless).
...
4. The replay at scale (T3-070, the Phase 3 exit criterion): record the 20k fight headless to its end, verify it on eight threads and watch it in the app.

```
cargo run --release -p il_cli -- autoresolve tests/scenarios/large/perf_20k.json5 --ai none --threads 8 --record-replay target/perf_20k.ilrp
cargo run --release -p il_cli -- replay target/perf_20k.ilrp --verify --threads 8
cargo run --release -p il_app -- --replay target/perf_20k.ilrp --threads 8
```

Expect `verified 6000 ticks` (or the tick the battle ended on) and a `final hash` line from the verifier, and `replay OK` in the app's title at the end of the playback (about five minutes at speed 1; the speed keys work in a playback). The nightly runs the first two commands every night and compares the final hash with a fresh `il_cli run` of the same file to the same tick (§8); the app playback is the check owed to your eyes.
...
- `autoresolve --record-replay <file>` also writes the battle's replay; `replay <file> --verify` re-simulates a replay (from the app or from `autoresolve`) with the loaded content and prints `verified N ticks`, or the first divergent tick with both hashes and exit code 1 (T2-101). Without `--verify` it prints the file's header (engine and schema versions, mods, content hash, time written, tick count). A replay written by different content is refused unless `--force`; `--threads 8` proves the recording on the parallel executor; `--verify` also prints the recording's `final hash` (T3-070), which `il_cli run --ticks N --hash-every N` of the same file must print on its last line. The nightly workflow records `ai_skirmish_300` and the 20k fight `large/perf_20k` (`--ai none`, so the file's scripted attacker stays; two engine armies never meet on the wide field) to their ends, verifies both on eight threads and compares the 20k recording's final hash with a second run.
