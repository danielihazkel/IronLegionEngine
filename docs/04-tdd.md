...
pub struct Hpa { abstract_astar: AStar, refine: AStar, graph: HpaGraph }   // Phase 3; as built in T3-020: `Hpa { graph: HpaGraph, astar: AStar }` with `new`, `graph()`, `rebuild(nav, &MovementRules)`, `ensure(nav, rules) -> bool` (rebuilds only when `graph.matches` fails: a swapped nav grid in a test, a world built by `empty`), `repair(nav, dirty)`; `PathfinderRes(pub Hpa)` since T3-020, and `serve_path_requests` calls `ensure` before `find`. Until T3-021 swaps the search in, `find` delegates to the plain A* so no path changes.
