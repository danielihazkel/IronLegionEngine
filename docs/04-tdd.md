...
pub mod interface {
    ...
    pub struct RegimentSetup { pub id: u32 /* campaign regiment id, echoed in result */, pub unit_type: ContentId,
        ...
        pub units: Vec<UnitGroupSetup { unit_type: ContentId, count: u16, experience: u8 }> /* T3-002 (SIM-FORM-012), built in T3-040: the ordered unit groups of a mixed regiment; `unit_type`, `count` and `experience` are `Option`s (the single-group shorthand, absent from the file when unset); `BattleWorld::new` rejects both forms together (`SetupError::BothUnitForms`), neither form (`EmptyComposition`), an unknown group unit (`UnknownUnit`) and a group of nobody (`GroupCountZero`); `RegimentSetup::single(id, unit_type, count)` and `mixed(id, units)` build either form, `groups()` normalises them, `total()` sums the counts, `experience_mean()` is the count-weighted mean, `first_unit()` the first group's; `form() -> Result<(), SetupForm>` */ }
    pub struct SetupWarning { pub side: u8, pub regiment: u32, pub text: String }   // T3-040: the setup check's soft findings (SIM-FORM-014's disjoint formations), kept by the world: `BattleWorld::setup_warnings()`; `il_cli run`/`autoresolve` print them, the app's event ring opens with them
    // `map_id` is required (T1-030); `BattleWorld::new` fails with `SetupError::UnknownMap`, `MissingDeploymentZone` or `PositionOutOfMap`.
    pub const SOLDIER_CAP: u32 = 32_768;                  // `BattleSetup::soldier_total()` (initial plus reinforcements) above it is `SetupError::OverCap` (SIM-CORE-006); so the arrival-time drop (`ReinforcementsDropped`) is unreachable from a valid setup and the setup check is the cap's proof (T3-025)
    ...
    pub struct RegimentResult { pub id: u32, pub initial: u16, pub survivors: u16, pub fled: u16, pub killed: u16, pub experience_gain: u16, pub ammo_left: u16, pub arrived: bool /* false for a reinforcement group that never entered, T2-070 */,
        pub units: Vec<UnitGroupResult { unit_type: ContentId, initial: u16, survivors: u16, killed: u16, fled: u16 }> /* T3-002 (SIM-FORM-015), filled since T3-041 from the living per group, `GroupTallies` and `UnitGroup.count`: one row per unit group in setup order, summing to the totals; a single-group regiment has one row; a never-arrived group lists its groups in full */ }
...
|---|---|---|---|
| `Soldier` | `id: SoldierId, regiment: RegimentId, unit: Handle<UnitType>, category: UnitCategory, group: u8` (T3-040: the index into `Regiment.units`, SIM-FORM-012, so two groups of one unit type reconcile in the result; identity like `id`, stored in `SoldierSnap.group`; the general rides in group 0) | id only | — |
...
pub fn label_slots(t: &FormationTemplate, slots: &mut [Slot], counts: &[(UnitCategory, u16)]);   // T3-040 (SIM-FORM-011/013): after `layout_slots`, labels the zone slots in rank-major order up to each category's count and clears the rest (and every slot outside a zone; a zoneless template labels nothing), so `assign_slots` (which matches `AssignSoldier.category` to `Slot.category`) keeps the zones and spills the overflow; called at spawn (counts from the groups plus the general), in `reform_one` (the living members) and in `rebuild_formation_derived`. The spawn assignment: soldiers in group order take the first free slot labelled their category, else the first free unlabelled slot.
...
pub fn terrain_defence_at(map: &LoadedMap, regs: &Registries, p_attacker: V2, p_defender: V2) -> S;  // the map-reading step the melee calls (defender's zone `defence_mult` and `ford`, both heights); T3-042 tests it on the ford, the bank and the bridge
...
| Determinism: each scenario twice, 1 thread and 8 threads, snapshot/restore at mid-point (the file's `determinism: { ticks, snapshot_at }` budget, default 10,000 / 5,000; `perf_10k.json5` runs 800 / 400 in the debug build, T2-111, and `large/perf_20k.json5` 400 / 200, T3-026; the band files 1,500 / 1,000 through the band harness's `SeedDriver` with their pins and harness applied, and identical rejection counts rather than none for the AI-driven files, T2-112) | `tests/tests/determinism.rs`, four tests (the classic scenarios, the band files, the 10k fight, the 20k fight), in-process on `BattleWorld` (`set_threads(1)` and `set_threads(8)`), plus an in-process `il_cli::run` twice comparison; CI also diffs two `il_cli run` logs per scenario (`idle_1000` and `move_reform_2000` for 10,000 ticks every 1,000, `perf_10k` for 1,500 every 100) | every push | REQ-TEST-002 |
...
| Group formations (T3-043): five scattered regiments sent through `GroupFormation` for every kind land on `arrange_group`'s placements with their facings and ranks, keep the arrangement's lateral order, and show the SIM-FORM-041/042 geometry (edge gaps of `group_gap`, the double line's `2 × gap`, the echelon steps, the refused flank `3 × gap` back at 45°, the skirmishers `skirmish_offset` ahead); an engine-owned side deploys the same five through the AI preset inside its zone with the same gaps, order, flank cavalry and forward skirmishers | `crates/il_sim_battle/tests/group_command.rs` (the UI preset: `crates/il_ui/tests/orders.rs`) | every push | REQ-FORM-009, REQ-FORM-010 |
| Mixed regiments, the fight and the result (T3-041): the band's cohort throws a volley and kills hoplites before the first `Engaged` and its result lists two rows summing to the totals; every scenario and band file reconciles one `UnitGroupResult` row per setup group with the regiment's totals; a builder row with two groups becomes a `units` composition the sim spawns as two groups and that round-trips through a saved file, and a group of nobody is a bad count; the result screen draws the sub-rows headless; the band `melee_mixed_cohort` at 50 seeds | `crates/il_sim_battle/tests/mixed.rs`, `tests/tests/result.rs`, `crates/il_ui/tests/custom_battle.rs`, `tests/scenarios/bands/melee_mixed_cohort.json5` (nightly) | every push (the band nightly) | REQ-FORM-008, REQ-SIM-061, REQ-UI-007 |
| Mixed regiments (T3-040): `tests/scenarios/mixed_cohort.json5` (40 velites and 100 hastati in `rome:cohort`, in the determinism and result corpora) spawns the velites in the front rank and the hastati behind with ids ascending by group, walks at the hastati's pace, keeps the zones through two rounds of deaths, snapshots and restores its groups, and the setup check rejects both forms, neither, an unknown group unit and an empty group while a repeated unit type stays two groups; a cohort whose groups share no formation (`tests/mods/velites_wedge_only`, the velites left only the wedge) takes the first list with one `SetupWarning`; `label_slots` and `Composition` unit tests; the identical hash logs at 1 and 8 threads; the hash test's `group tallies` case | `crates/il_sim_battle/tests/mixed.rs`, `src/formation/layout.rs`, `src/composition.rs` | every push | REQ-FORM-008 |
| Rivers, fords and bridges (T3-042): a regiment in the ford walks at `move_mult` 0.5 (anchor and soldiers), the melee's terrain step gives 0.7 for a defender in the ford and 1 on the bank and the bridge, a melee resolves in the ford, a 24 m line crosses the 8 m bridge in Column on an HPA\* path whose river cells all lie under the bridge, a regiment ordered across an uncrossable stretch goes round by the bridge, and a shoved line never enters a river cell; every scenario at 1 and 8 threads with equal hashes | `crates/il_sim_battle/tests/rivers.rs` | every push | REQ-SIM-042, REQ-SIM-040 |
...
| Replay at scale (T3-070) | `il_cli autoresolve tests/scenarios/large/perf_20k.json5 --ai none --threads 8 --record-replay F` to the battle's end, `il_cli replay F --verify --threads 8` (prints `verified N ticks` and `final hash H`), then `il_cli run` of the same file for `N` ticks whose last hash line must be `N,H`; the app playback (`--replay F`, `replay OK`) is the docs/08 §4g check | nightly | REQ-SAVE-005, Phase 3 exit criterion |
