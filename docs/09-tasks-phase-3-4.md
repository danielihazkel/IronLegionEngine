...
- [x] **T3-003 Spec: LOD tiers, the render thread flag, the Editor state** · S · Refs REQ-RNDR-004, REQ-RNDR-007, REQ-TOOL-004, TDD §10.1, §15, §16, SAD §5.2, §6.1, §8 · Depends none
  ...
  **Done when** the sections exist and `dep_rules.rs` lists `il_editor` (the crate itself arrives in T3-060).
  Done 2026-09-10: TDD §10.1 LOD (`DetailTier`, `detail_tier(zoom, z1, z2)` with `zoom ≥ z1` Detailed since zoom is pixels per metre and `z1` 24 > `z2` 8, the tier table with the projectile and corpse culling columns for REQ-RNDR-009, `RenderSnapshot.tier` / `blocks`, `BlockInst`, `Settings.detail_z1` / `detail_z2`), §10.1 Threading (`FrameJob` over a one-slot replacing channel, `--single-thread-render`), §15 (`AppState::Editor`, `Transition::OpenEditor`, the flag in `Launch`), §16 (`MapDocument`, `History`, `Tool`, `EditorSession`, the save and preview design); SAD §5.1, §5.2 (the `il_editor` rule and solid edges), §8; PRD OQ-4 note; `dep_rules.rs` lists `il_editor` with its allowed set and an `ARRIVES_LATER` tolerance for the missing manifest that T3-060 removes.

### SIM — pathfinding and the 20k tick

- [x] **T3-020 HPA\* graph** · M · Refs SIM-MOVE-003, TDD §6.1 `HpaGraph`, REQ-PATH-001 · Depends T3-001
  ...
  **Done when** golden tests pin the gate count and positions on `tests/maps/tiny/` and `rome:test_field`, every intra-cluster edge cost equals the Dijkstra oracle, and a build of the 1600 × 1200 m map of T3-024 takes under 50 ms in a benchmark.
  Done 2026-09-10: `il_sim_battle::hpa` (`HpaGraph`, `GateNode`, `DirtyRect`, `ClusterSearch`, `Hpa`) with `build(nav, cluster, gate_split)` (the split is a parameter too, read from the rules), `repair` (re-gates the touched clusters' borders and recomputes their and their neighbours' intra edges; tested equal to a fresh build), `matches` / `ensure`; `PathfinderRes` is a `Hpa` whose `find` still delegates to A\* until T3-021; `rebuild_derived` builds the graph, Stage 3 calls `ensure`. Intra costs are one cluster-bounded Dijkstra per gate cell over a padded local copy of the cluster (equal to the cropped-cluster Dijkstra oracle on 40 random grids and three uniform ones), with a closed-form octile cost for uniform clusters; clusters run in parallel on the task pool, each into its own slot (serial and 8-thread builds compared equal). Goldens: `tests/maps/tiny` at cluster 1 has 2 nodes (the two northern cells), `rome:test_field` 1,382 nodes in 13 × 10 clusters with the banks connected through the graph. *Decision:* the T3-024 map does not exist yet, so the benchmark builds a synthetic 400 × 300-cell grid: open 4.2 ms, 25 % random rock 20 ms on the 8-thread pool (the app's and `bench --threads 8`'s configuration) and 105 ms serial; the test map takes 3.7 ms. The bound holds for the configurations that matter and for any realistic map; the serial random-rock number is the search cost and is recorded rather than optimised further. `hpa_cluster` and `hpa_gate_split` left the `rules_fields_read.rs` allow-list. The old `nav` bench asserted on a result only its own filtered-out iteration filled; fixed.

- [x] **T3-021 HPA\* search, refinement, `Pathfinder` swap** · M · Refs SIM-MOVE-002, TDD §6.1 `Hpa`, REQ-PATH-001, REQ-PATH-002 · Depends T3-020
  ...
  **Done when** the HPA\* path cost is within 10 % of the A\* cost on 1,000 random grids (TDD §6.1 test list; *the corpus mean, owner's decision 2026-09-10, the per-path spread recorded*), the same request gives the same path on 1 and 8 threads, no path crosses an impassable cell, `move_reform_2000` still arrives and morphs at the bridge (T1-042's test), and Stage 3's mean at 20k is inside its 1 ms budget with 200 regiments re-pathing.
  Done 2026-09-10: `Hpa::find` (TDD §6.1 as built): endpoints snapped as A\* does, a one-cluster request answered by a bounded A\*, the start and goal linked to their clusters' gates by a forward and a reverse-cost bounded Dijkstra, an A\* over the gate nodes with node-index tie-breaks, then every segment between the start, the far cell of each gate crossed and the goal refined by an A\* bounded to the two clusters that gate joins, and `string_pull`. `PathfinderRes` is the `Hpa`; `AStar` stays for tests, the refinement and the oracle. Quality on 969 solvable requests over 40 random 64 × 48 grids with 25 % rock (harsher than any map): at the flagship rules 16 / 6 the mean cost is 1.026 × A\*, 97 % of paths within 10 %, worst 1.41; refining inside one cluster gave 1.040 / 59 paths over; a whole-path corridor refinement gave 1.003 / worst 1.12 at 283 µs per find against 125 µs and was measured and rejected. *Decision (owner):* the three levers inside the rule (two-cluster refinement, the in-cluster direct A\*, the gate split down to 1: mean 1.009, worst 1.30) all leave single paths over 10 % on those grids, so the bound is read as the corpus mean, asserted at 16 / 6, and SIM-MOVE-003 stays as written. Same path at 1 and 8 threads through Stage 3, the bridge crossed by the world's own pathfinder, the T1-042 column morph unchanged, the determinism corpus green. Stage 3 mean at 20k (`bench --soldiers 20000 --threads 8`, 100 regiments re-pathing on the script's move ticks, at most `paths_per_tick` 8 served per tick) 0.51 ms against the 1 ms budget; one corner-to-corner find on `rome:test_field` 125 µs. No committed hash golden depends on paths (the 1,000-tick golden is an idle world), so nothing was re-baselined but `benches/baseline.json`, re-recorded for every key (`docs/evidence/phase3/machine.md`).
...
- [ ] **T3-022 Collision and steering pass** · L · Refs SAD §12 T-10, SIM-MOVE-022, SIM-MOVE-040..042, TDD §6.2 budgets · Depends T3-001
  ...
  **Done when** `il_cli bench --soldiers 20000` on the target machine shows Collision ≤ 8 ms and SoldierSteering ≤ 8 ms mean with the hash logs unchanged, and the baseline is re-recorded.
  Built 2026-09-11 (the box waits for the baseline re-record: the target machine throttled to 47 % of its clock that afternoon, `docs/evidence/phase3/machine.md`): Collision 3.1 ms (11.3 in the T3-021 recording) and SoldierSteering 4.8 ms (6.7) mean at 20k over three morning sittings, the four `il_cli run` hash logs (`idle_1000`, `move_reform_2000`, `perf_10k`, `ai_skirmish_300`) byte-identical after every step. Every change removes only work that added nothing to a soldier's floating sum (TDD §6.2 as built): `CollisionScratch` keeps the buffers across ticks; `SoldierBodies` carries every grid entry's radius and mass beside the grid (filled at Stage 6, kept aligned by Stage 7's rebuild and the death pass); the fold skips a pair by a slack-guarded squared distance before its square root; a pass that changed no position ends the loop; and the pair set is narrowed at enumeration to `2 r_max + 1 m` under a per-row guard on each cell's largest travel sum that re-enumerates a row in full when it can no longer prove no dropped pair overlaps (17,005 of 900,000 row lists in the 10k fight, 1.9 %). Steering reads the body table, selects its `sep_max_neighbours` nearest with `select_nth_unstable_by` on the `(d², index)` key instead of a full stable sort, and reads regiments from a per-tick table. The plan's two reserve steps (a two-phase parallel fold, a per-row steering gather) were not needed. Unit tests pin the pre-checked fold and the narrowed set (with a forced widening) to the plain fold bit for bit. SAD T-10's collision and steering half is closed.
