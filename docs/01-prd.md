...
|---|---|---|---|
| OQ-1 | Camera rotation: 4 snap directions (4 sprite facing sets suffice at cost of diagonal facings) or 8 (8 facing sets, more art)? **Resolved (Phase 1, T1-052): 4 camera snaps with 8 sprite facings. `Camera.rotation` is a quarter turn 0..=3 and the drawn facing is `(facing8 − 2·rotation) mod 8`, so one 8-row sheet serves every snap and diagonal facings are kept; 8 snaps stay a Could.** | TDD §10 | Phase 1 |
...
| OQ-4 | Should far-from-combat regiments update at reduced tick rate (simulation LOD) if R-1 materialises? Phase 3 decision (T3-003): not built unless T3-024 misses the 20k tick budget; T3-080 records the outcome. | Simulation Spec | Phase 3 |
...
| OQ-8 | Audio crate: `kira` (game-oriented) or `rodio`? **Resolved (T2-100, 2026-09-07): `kira` 0.12.4 in `il_audio`, sound sets as content (`content/sounds/`, Modding SDK §4.14), placeholder samples written by `il_cli gensound`; the sim has no audio dependency (`dep_rules.rs`).** | TDD §12 | Phase 2 |
