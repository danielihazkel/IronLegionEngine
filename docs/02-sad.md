...
| T-12 | `il_sim_campaign` and `game/rules` are empty placeholder crates, so several §5.2 edges cannot be enforced yet (`il_ai` gained its code and its `il_core` / `il_data` edges in T2-080; `il_save` gained the container, the replay and battle-save bodies and its `il_core` / `il_data` / `il_sim_battle` edges in T2-101) | Debt | workspace | Filled by their phases; `dep_rules.rs` already lists them so the rules apply the moment they gain dependencies. |
