...
| `il_audio` | Event-driven sound playback (the event router with rate limits and a camera listener), zoom mixing between the roar loop and the individual effects, the kira engine; music state arrives with Phase 4 | REQ-AUD-001, 002 |
...
| T-5 | Sim on main thread until Phase 3 | Debt | il_app | `RenderSnapshot` (owned data, built after stepping) exists since T1-052, so moving to a render thread is plumbing only. Phase 2's 10k fight ticks at 13 ms on the main thread, so every third frame at 60 FPS carries it (T2-111/113): the render thread is the first Phase 3 item if the owner's `profiler_10k.png` shows the frame rate paying for it. |
...
| T-10 | At 20k soldiers `Collision` (12.1 ms) and `SoldierSteering` (7.8 ms) sat over their Phase 3 budgets of 8 ms, and `Targeting` (3.0 ms at 20k with no enemy in sight) pays its per-soldier pass whether or not anyone fights (TDD budget table; T1-083, re-measured in T2-111) | Closed (T3-022, T3-023) | il_sim_battle | T3-022 brought Collision to 3.0 to 3.5 ms and SoldierSteering to 4.7 to 5.8 ms at 20k with every hash log unchanged (scratch buffers and a body table reused across ticks, a squared pre-check before the square root, an idle-pass exit, and the pair set narrowed to `2 r_max + 1 m` under a displacement guard that widens it when it cannot prove completeness; TDD §6.2). T3-023 made `melee_recount` walk the gated regiments' lists only and zero only the soldiers counted last tick (TDD §8.1); Stage 9 at 20k with no enemy in sight fell from 3.3 ms to the figure in the note under the TDD budget table. |
| T-11 | Lint opt-outs: `il_render` (`unsafe_code = "deny"` for the wgpu surface, `float_arithmetic = "allow"`), `il_ui`, `il_audio` and `il_app` (`float_arithmetic = "allow"`), `il_cli::gensound` (`float_arithmetic` allowed inside the module: tool-side DSP); `il_cli::bench::StageTimer` allows `Instant::now`; `il_cli` carries its own `[lints]` table (`unsafe_code = "deny"`) for the one `#[allow(unsafe_code)]` Win32 call of `bench --memory` (T3-025) | Debt | presentation crates | Each carries its reason in the manifest or attribute; the render-side float allowance is by design (no sim arithmetic there). The unused `tracing` declaration in `il_sim_battle` was dropped in T3-001. |
| T-12 | `il_sim_campaign` and `game/rules` are empty placeholder crates, so several §5.2 edges cannot be enforced yet (`il_ai` gained its code and its `il_core` / `il_data` edges in T2-080; `il_save` gained the container, the replay and battle-save bodies and its `il_core` / `il_data` / `il_sim_battle` edges in T2-101; `il_audio` gained the router and the kira engine and its three edges in T2-100) | Debt | workspace | Filled by their phases; `dep_rules.rs` already lists them so the rules apply the moment they gain dependencies. |
