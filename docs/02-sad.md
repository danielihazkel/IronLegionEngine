...
| T-9 | Ten of the 18 stages (1, 8, 11..16 (Stages 9 and 10 got their systems in T2-020 and T2-021)) run an empty placeholder system so the profiler shows every row; together they cost ≈ 0.3 ms of schedule overhead per tick at 20k | Debt | il_sim_battle | Each placeholder is replaced by its real systems in Phase 2; if any stage stays empty after that, drop its schedule. |
