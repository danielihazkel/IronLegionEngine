...
| T-10 | At 20k soldiers `Collision` (12.1 ms) and `SoldierSteering` (7.8 ms) sit over their Phase 3 budgets of 8 ms, and `Targeting` (3.0 ms at 20k with no enemy in sight) pays its per-soldier pass whether or not anyone fights (TDD budget table; T1-083, re-measured in T2-111) | Risk | il_sim_battle | Phase 2 combat did not grow them (the 10k fight's tick is 13.3 ms against the 25 ms P2 bar); Phase 3 candidates are a narrower pair neighbourhood, fewer `collision_iterations` when nobody moved, per-row buffers reused across ticks, and a targeting pass over the gated regiments' soldier lists only. |
